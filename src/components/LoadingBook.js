...
const LoadingBook = ({ id, volumeInfo }) => {
  const imageVariants = {
    hover: {
      scale: 1.7,
  ...
  let { title, authors, publisher, previewLink, imageLinks, description } = volumeInfo;
    ...
    >
      <Flex flexDirection={{sm: 'column', md:'row'}} gap="5">
        ...
        <Box>
          ...
          <Box>
            {authors && (
              <Heading marginBottom={".3em"} as="h4" size="sm" fontWeight={'medium'}>
                {' '}
                Author:{' '}
                <span
                  style={{
          ...
          <Box>
            {description && (
              <Text>{description.substr(0, 150)}...</Text>
            )}
          </Box>
          {/* Google books link */}
          <Box>
